...
## Start with a simple example. The agent can be in three states,
## which are adjacent rooms. By going left or right the agent can move
## from one room to another, if there is a room, or take a no-op and
## stay where it is.

## The map of the task looks like this:

## s1 - s2 - s3

## The agent starts from s1, and needs to go to s3 (by going right
## twice, as we can see from the map). When the agent reaches s3, it
## is rewarded.
...
class mdp():
    ...
    def __init__(self):
        ...
        # The state is a value that distinguishes the current state
        # from the others.
        self.state = "s1"

        # This flags if a terminal state has been reached, prompting a
        # new task to be generated by calling self.reset().
    ...
    def iterate_model(self, print_progress = False, force_action = None):
        ...
        # Observe the current state. Here, it is just the identity.
