...
class driver(pomdp.pomdp):
    ...
    def learn_transitions(self, iters, output_progress = False):
        ...
        for a in self.actions:
            ...
            for pos in self.possible_positions:
                # Finer discrete positions within the represented
                # belief. There is actually a slight bug here in the
                # discretisation, but the final model works
                # nevertheless.
    ...
    def calculate_reward(self):
        ...
        # penalty gets larger with larger positions.
        ...
        if self.action != None and has_attention == True:
            ...
            for b in self.pos_belief.keys():
                ...
                if self.resolution*b == round(observed_pos*self.resolution):
                    ...
                    # other positions
                    self.pos_belief[b] = ((1-self.obs_prob) / (len(self.pos_belief) - 1)) * self.pos_belief[b]
...
# d.noise = 0.01
# d.action_noise = 0
# # Learn the transition table: given current state and
# # action, what is the new state distribution.
# d.learn_transitions(1000, output_progress = True)
# d.log_data = True
# # learn the model
# d.run_model(100000, output_progress = True)
# d.write_data_to_file("driver22.csv")
